...
from ..deps_auth import require_doc  # صلاحية طبيب
...
def export_rest_notice_by_visit(
    ...
    # استخرج عدد الأيام من rec_json أو rest_days (دعمًا للحالات القديمة)
    days = None
    rec = None
    try:
    ...
    # الشعار — نُمرّر URL نسبي داخل /static/ ليُعالجه link_callback
    logo_src = None
    ...
    payload = {
        ...
        "shape": _shape_ar_safe,  # نفس المعالجة للنصوص العربية
    }
...
def _clean_treatment_from_notes(notes: str | None) -> str | None:
    ...
    for i, ln in enumerate(lines):
        # أول سطر غالباً يحوي الإضافات المجمّعة: الوزن/الطول/BMI/سكر/O₂
        if i == 0 and any(k in ln for k in ("الوزن", "الطول", "BMI", "O₂", "سكر الدم")):
            continue
        # سطر الأمراض المزمنة الآلي
...
def _get_patient_card(db: Session, patient_key: str):
    ...
    try:
        ...
        try:
            ...
            # محاولة الحصول على البيانات من ملف Excel
            if ptype == "T":
                try:
                    import sys
                    import os
...
def visit_create(
    ...
    try:
        ...
        # الحرارة إجبارية ضمن مجال منطقي
        ...
        # ===== الأمراض المزمنة (تحقق) =====
        chronic = chronic or []
        # طَبّع القيم لإزالة فراغات واحترام العربية
        chronic_norm = []
        for c in chronic:
        ...
        # ===== JSON للأمراض المزمنة لتخزينه في chronic_json =====
        ...
        # ===== دمج الملاحظات (BMI + الوزن/الطول + سكر الدم + O₂ + الأمراض المزمنة) =====
        ...
        chronic_note = None
        if chronic_norm:
            # أعرض الأمراض المختارة بالعربية + إلحاق (أخرى: ...) إن وُجد
    ...
    try:
        uid = (request.session.get("user") or {}).get("id")
        ptype, pno = parse_patient_key(patient_key)
        # ... (منطق مطابق، تُرك معلقًا للحفاظ على الطول)
    except Exception as ex:
        db.rollback()
