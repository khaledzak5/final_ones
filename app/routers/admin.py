...
def admin_home(request: Request, db: Session = Depends(get_db), msg: Optional[str] = Query(None)):
    ...
    # إحصائيات الدورات التدريبية - عدد المسجلين في الدورات المنشورة فقط
    try:
        # عدد المسجلين في الدورات المنشورة
    ...
    try:
        ...
        # التحقق من وجود جدول clinic_patients
        from ..database import is_sqlite
        
        if is_sqlite():
            # SQLite - استخدام LIKE للبحث في JSON
            # عدد المراجعين (الزيارات)
            ...
            # عدد الإجازات (من rest_days أو recommendation = 'rest')
        ...
        # في حالة عدم وجود جدول clinic_patients أو أي خطأ آخر
        # جرّب من الإكسيل
        try:
            import sys
            import os
            ...
            counts["doctors"] = 1  # طبيب واحد على الأقل
            counts["visits"] = stats.get('total_clinic_patients', 0)
            counts["referrals"] = int(stats.get('total_clinic_patients', 0) * 0.1)  # تقدير تقريبي
            counts["leaves"] = int(stats.get('total_clinic_patients', 0) * 0.05)
            counts["excel_source"] = True  # علامة تشير أن البيانات من الإكسيل
        except Exception:
            counts["doctors"] = db.query(User).filter(User.is_doc == True, User.is_active == True).count()
            counts["visits"] = 0
            counts["referrals"] = 0
            counts["leaves"] = 0
    # نسبة الدورات المنشورة
    try:
...
def admin_audit(request: Request, db: Session = Depends(get_db)):
    ...
    # الحصول على إحصائيات الدخول لكل مستخدم
    # نستعلم عن آخر دخول وعدد مرات الدخول لكل مستخدم
    ...
    # ترتيب حسب آخر دخول (الأحدث أولاً) - استخدام datetime.min كقيمة افتراضية
