...
from ..models import Department, College   # لا نعدّل الأقسام، نقرأ منها فقط
...
def ensure_unique_name(db: Session, name: str, exclude_id: Optional[int] = None) -> bool:
    ...
    # جلب جميع الكليات وتطبيع أسمائها في Python (لأن SQLite لا يدعم regexp_replace)
...
# -------- List (merge view: colleges ∪ distinct(departments.college)) --------
@router.get("")
@router.get("/")
def colleges_list(
    request: Request,
    ...
    # إذا كان أدمن كلية، عرض كلية واحدة فقط
    ...
    # لا نضيف عناصر افتراضية من الأقسام لأدمن الكلية إلا إن كانت ضمن كليته
...
def college_new_form(
    ...
    # منع أدمن الكلية من إنشاء كلية جديدة
...
def college_create(
    ...
    item = College(
        name=name_n,
        name_en=name_en_n,
        name_print_ar=(name_print_ar_n or name_n),  # افتراضي: ينسخ الاسم العربي الأساسي
        dean_name=dean_n,
    ...
    db.commit()
    # إعادة توجيه لصفحة الأدمن مع رسالة نجاح
...
def college_edit_form(
    ...
    # إن كان أدمن كلية، فلا يحق له تعديل كلية غير كليته
    cu = get_current_user(request, db)
...
def college_toggle(
    ...
    item = db.get(College, cid)
    # إن كان أدمن كلية، فلا يحق له تبديل حالة كلية غير كليته
    if item:
        ...
        # التفعيل يعتمد فقط على اسم الكلية
...
def college_delete(
    ...
    if not item:
        return RedirectResponse(url="/admin/colleges", status_code=status.HTTP_303_SEE_OTHER)
    # منع أدمن الكلية من حذف الكلية — الحذف للسوبر أدمن فقط
